    ...
    for(let x = 0; x < columns; x++){
      for(let y = 0; y < rows; y++){
        this.invaders.push(
          new Invader({
            position: {
              x: x * 30, //30 é o tamanho do invader
              y: y * 30
            }
          })
        )
...
let frames = 0
let gridFrameRate = Math.floor(Math.random() * 500 + 500)
let game = {
  over: false,
  active: true
}
...
function animate(){
  ...
  //spawning enemies grids
  if (frames % gridFrameRate === 0) {
    grids.push(new Grid())
    gridFrameRate = Math.floor(Math.random() * 500 + 500)
    frames = 0
  }
